	...
	<td colspan="4" id="user-list-{$titleHash}" style="display:none">
		...
		{foreach from=$tableHash.info.users item=userHash}
			<li class="item width100p" style="border-bottom:1px solid #cccccc;">
				...
				<div class="inline-block width65p">{if $userHash.referer_url}<a href="{$userHash.referer_url|escape}">{$userHash.referer_url|stats_referer_display_short}</a><br/>{/if}{$userHash|getDisplayLinkFromHash:1} - {$userHash.email}</div>
